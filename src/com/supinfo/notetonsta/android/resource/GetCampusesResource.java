...
public class GetCampusesResource implements Runnable {
	...
	public void run() {
		...
		message.arg1 = GetCampusesHandler.STATUS_STARTED;
		...
		try {
			...
			parseJSON();
			regenerateAdapter();
			message.arg1 = GetCampusesHandler.STATUS_FINISHED_OK;
		} catch(ConnectTimeoutException e) { 
			Log.w("Warning",e.getMessage(),e);
			message.arg1 = GetCampusesHandler.STATUS_ERROR_TIMEDOUT;
		} catch(HttpHostConnectException e) {
			Log.w("Warning",e.getMessage(),e);
			message.arg1 = GetCampusesHandler.STATUS_ERROR_CONNREFUSED;
		}  catch (JSONException e) {
			// This will be caught if JSON library can't parse server return
			Log.w("Warning",e.getMessage(),e);
			message.arg1 = GetCampusesHandler.STATUS_ERROR_JSON;
		} catch(Exception e) {
			// We've to catch any unknown exception
			Log.e("Error",e.getMessage(),e);
			message.arg1 = GetCampusesHandler.STATUS_ERROR_UNKNOWN;
		} finally {
			handler.sendMessage(message);
		}
	}
